...
//  ESE CÓDIGO UTILIZARÁ EL COPROCESADOR Y SERÁ ALGO MÁS RÁPIDO (CUANTO)
    ...
    *COPput = 0;
    ...
    // Desenmascaramos la respuesta
    nBytesOut = resp >> 24;
    // bytesOut = resp & 0xFFFFFF; //No se usa (?)

    while(nBytesOut--){
        fputc(resp & 0xFF, Hfile_out);
        resp >>= 8;
    }
...
//  ESE CÓDIGO UTILIZARÁ EL COPROCESADOR 
    ...
    // Enviamos el codigo de terminación
    *COPput = 64 << 16;
    ...
    *COPput = 0;
    ...
    // En el ultimo solo puede recibir un byte
    while(nBytesOut--){
        fputc(resp & 0xFF, Hfile_out);
        resp >>= 8;
    }
